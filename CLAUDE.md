...
- Platform: Cloudflare Pages(静的 PWA、`dist/` をホスト)。GitHub repo: `https://github.com/kimymt/squisher`(public、デフォルトブランチ `main`、branch protection で PR 必須)
...
- Deploy workflow: Cloudflare Pages の **Git 連携**(feature ブランチで PR → CI green → squash merge to `main` → CF が `npm run build` → `dist/` を auto-deploy)。PR ブランチには自動でプレビューデプロイ。merge → 30〜90 秒で本番反映。
- Deploy status command: HTTP ヘルスチェック
- Merge method: 直接 push は protection rule で不可。**feature ブランチ → PR → CI green → squash merge to `main`** が production。CI `.github/workflows/test.yml` が push/PR で走る。
...
- Deploy trigger: PR を `main` に squash merge(Git 連携で auto-deploy)

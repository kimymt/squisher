...
>
> **進捗(2026-05-13):** Phase 0–4 ✅ 完了、Phase 5 🚧 進行中(Vitest 基盤 + 単体22件 + `/qa` 通し済み、Playwright E2E と実機検証が残り)。`master` ブランチ、`634333c`→`b70cd1e`。実機が無いため「最短2タップ / 10ファイル25-30秒」は未測定。ホスティング未決定。
...
| 画像圧縮 | **native Canvas `toBlob()`**(`compress.ts`) | 依存ゼロ、iOS Safari 実績、品質パラメータは `toBlob(type, quality)` で十分。`browser-image-compression` は dep に追加済みだが未使用(将来の品質チューニング用に保持) |
...
vitest.config.ts            # テスト専用(preact のみ、jsdom 環境、test/setup.ts)
...
test/                       # setup.ts + output-format / signals / install / app-preset.regression-1 の .test.ts
.github/workflows/test.yml  # npm ci → npm test → npm run build(push / PR)
```
...
- ✅ 単体テスト 22件 全パス: `output-format`(detect/mime/ext)、`signals`(totalCompressedSize / saveableFiles / canSave の `skipLarger×larger` 分岐、addFiles / updateFile / nextId)、`install`(`shouldOfferInstall` の iOS/standalone/dismiss、`dismissInstallBanner` の storage 失敗)、`app-preset.regression-1`(ISSUE-001 回帰)。
- ✅ `/qa` 通し(ヘルススコア 98→100): 発見2件・両方修正(ISSUE-001 プリセット再圧縮 `5d86443`、ISSUE-002 375pt ツールバー折返し `89e0ac1`)。レポート `.gstack/qa-reports/`。
- ⬜ 残り: コンポーネントテスト(FileRow / SaveBar / Header / InstallBanner)、`compress.ts` の実ブラウザ検証(jsdom に canvas なし → Playwright)、Playwright E2E(7 user flows)、ダークモード切替の見た目、visibilityChange 中の挙動。
- ⬜ **実機(端末待ち)**: 全プリセット・複数ファイル・共有シート・ホーム画面追加、「最短2タップ」「10ファイル 25-30秒」の測定、`navigator.share` 成功/キャンセルの実挙動、ダウンロードフォールバックの iOS 挙動。
...
- **Unit (Vitest)**: `compress.ts` は jsdom に canvas/createImageBitmap が無いため実ブラウザ(Playwright)側で検証。`decode-heic.ts` / `detect-transparency.ts` は Phase 0/PLAN レビューで削除済み(該当テストなし)。
  ...
  - ⬜ `share.ts`: `canShare→share` happy / `canShare false→fallback` / AbortError→cancelled / 非Abort→failed(`navigator.share` をモック)
  - ⬜ `presets.ts`: 各プリセット値検証 / `result.ts`: 型ユーティリティ
  - ⬜ `compress.ts`(Playwright): happy(JPEG / PNG)+ edge(巨大 36Mpx / 64Mピクセル超 / 破損 / サイズ増)、サムネ生成
...
- **CI (GitHub Actions)** — ✅ `.github/workflows/test.yml`(push / PR で `npm ci → npm test → npm run build`、`npm run build` = `tsc && vite build` なので型チェック込み)
  - ⬜ Playwright(iOS Safari エミュレーション、ヘッドレス)を CI に追加(Phase 5 で)
...
4. **ホスティング — 未決定**。Cloudflare Pages 推奨(HTTPS 必須、PWA 配信実績、無料 tier 十分)。次のアクション: ホスティング選定 + `/setup-deploy`。
5. **`browser-image-compression` の扱い — 未決定**。dep に追加済みだが `compress.ts` は native Canvas `toBlob` を使用しており未 import。品質チューニングで使うなら活用、使わないなら `npm uninstall`(現状はバンドルに含まれないが node_modules / package.json には残っている)。
...
| 2026-05-13 | 圧縮コアは native Canvas `toBlob`、`browser-image-compression` は未使用 | 依存ゼロで `toBlob(type, quality)` で十分。dep は将来の品質チューニング用に保持(未 import) |

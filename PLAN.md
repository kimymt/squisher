...
>
> **進捗(2026-05-13):** Phase 0–4 ✅ 完了、Phase 5 もほぼ完了(`npm test` 51件・11ファイル + Playwright E2E 8件 + `/qa` 通し済み、残るは実機検証のみ)。`master` ブランチ。デプロイ先は **Cloudflare Pages**(CLAUDE.md の `## Deploy Configuration` 参照、初回 `wrangler login` + `wrangler pages deploy dist` が未実施)。実機が無いため「最短2タップ / 10ファイル25-30秒」と standalone モードは未測定。
...
4. ~~ホスティング~~ → **Cloudflare Pages に決定**(`/setup-deploy` で CLAUDE.md の `## Deploy Configuration` に記録、2026-05-13)。デプロイは `npm run build && npx wrangler pages deploy dist --project-name=squisher`。初回のみ `wrangler login` + プロジェクト作成。GitHub repo を繋げば push で auto-deploy + CI。
